		...
		void Update()
		{
			...
			if (_mediaPlayer.Info != null)
			{
				...
				if (_timelineTip != null)
				{
					...
					{
						RectTransformUtility.ScreenPointToWorldPointInRectangle(_canvasTransform, Input.mousePosition, Camera.main, out Vector3 canvasPositionInRectangle);

						_segmentsSeek.gameObject.SetActive(true);
						_timelineTip.gameObject.SetActive(true);
						
						_timelineTip.position = new Vector2(canvasPositionInRectangle.x, _timelineTip.position.y);

						if (UserInteraction.IsUserInputThisFrame())
						{
							RectTransformUtility.ScreenPointToLocalPointInRectangle(_canvasTransform, Input.mousePosition, Camera.main, out Vector2 canvasPosition);
							...
							float x = Mathf.Clamp01((canvasPosition.x - bounds.min.x) / bounds.size.x);
				...
				if (_mediaPlayer.Info.GetDuration() == _mediaPlayer.Control.GetCurrentTime())
				{
					Pause(true);
				}
